...
const updateUserAvatar = asyncHandler(async(req, res) => {
    ...
    const avataUpdate = await User.findByIdAndUpdate(
        req.user?._id,
        {
            $set:{
                avatar: avatar.url
    ...
    .status(200)
    .json(
        new ApiResponse(200, avataUpdate, "Avatar image updated successfully")
    )
})
...
const updateUserCoverImage = asyncHandler(async(req, res) => {
    ...
    const deleteCoverImage = await User.coverImage?.url 
    const coverImage = await uploadOnCloudinary(coverImageLocalPath)
